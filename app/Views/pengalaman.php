...
<head>
    ...
    <title><?= $title ?? 'Riwayat Pengalaman' ?></title>
...
<body class="text-slate-200 p-4 md:p-8">
    ...
    <div class="max-w-7xl mx-auto bg-slate-800 rounded-2xl shadow-2xl flex flex-col md:flex-row min-h-[80vh] overflow-hidden">
        ...
        <main class="flex-grow p-6 md:p-10 bg-slate-900 rounded-b-xl md:rounded-r-xl md:rounded-b-none overflow-y-auto">
            <section class="mb-8">
                <h2 class="text-3xl font-bold text-white mb-6">Riwayat Pengalaman</h2>
                    ...
                    <?php if(!empty($pengalaman)): ?>
                        <?php foreach($pengalaman as $item): ?>
                            <div class="relative">
                                ...
                                <h3 class="text-xl font-semibold text-white"><?= esc($item['posisi']) ?></h3>
                                <p class="text-lg text-blue-300"><?= esc($item['nama_perusahaan']) ?></p>
                                <?php if(!empty($item['lokasi'])): ?>
                                    <p class="text-md text-slate-200 mt-1"><?= esc($item['lokasi']) ?></p>
                                <?php endif; ?>
                                <?php if(!empty($item['deskripsi'])): ?>
                                    <p class="text-slate-300 mt-2"><?= nl2br(esc($item['deskripsi'])) ?></p>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="text-center text-slate-400 mt-6">💼 Belum ada data pengalaman</p>
                    <?php endif; ?>
                </div>
            </section>
        </main>
